...
int main(){
	...
	double size_sq = sqrt(size);
	size = sqrt(size);
	if(size == size_sq){                   //Checks if the size is a perfect square.
	...
	*/
		...
		for(int j = 0; j < sqrt(size); j++){
			for(int k = 0; k < 4; k++){
				if(!input.eof()){
					...
					cout << "End of file reached before expected. Terminating program." << endl;
					system("PAUSE");
					return 0;
				}
			}
	...
	cout << "For safety, any possible extra gate values have been truncated." << endl;
